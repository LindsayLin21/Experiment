...
def validate(epoch, config, model, valid_loader, save_fname, best_acc=0):
    correct = 0
    total = 0
    test_loss = 0
    ...
    if test_acc > best_acc:
        ...
        torch.save(model, os.path.join(config.train.output_dir, save_fname))
        best_acc = test_acc
...
def main(args):
    ...
    if config.train.resume and config.train.checkpoint != '':
        checkpoint = load_checkpoint(config.train.checkpoint, device='cpu')
    ...
    for epoch in range(config.train.epochs):
        ...
        best_acc = validate(epoch, config, model, valid_dataloader, args.save_fname, best_acc=best_acc)
