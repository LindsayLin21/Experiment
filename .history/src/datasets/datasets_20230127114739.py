...
from src.transforms import create_transform
        ...
        if is_train:
            if config.train.use_test_as_val:
                ...
                train_dataset = module(config.dataset.dataset_dir,
                                       ...
                                       download=True)
                val_dataset = module(config.dataset.dataset_dir,
                                      train=False,
            ...
            if config.train.select == 'random':
                ...
                if config.train.select.class_list == []:
                    target_classes = np.random.choice(
                        ...
                        replace=False)
                    config.defrost()
                    config.train.select.class_list = target_classes
                    config.freeze()
                else:
